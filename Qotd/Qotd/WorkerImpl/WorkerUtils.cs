    ...
    public static class WorkerUtils
    {
        ...
        public static void AggregateNotifications(this QotdContext db)
        {
            using (TransactionScope scope = CreateTransactionScope())
            {
                ...
                {
                    using (TransactionScope scope2 = CreateTransactionScope())
                    {
                        var ns = un.ToArray();
        ...
        {
            Activity[] activities;
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = System.Transactions.IsolationLevel.ReadUncommitted, Timeout = new TimeSpan(1, 0, 0) }))
            {
                // create links
                ...
                activities = db.Activities.Where(a => !a.NotificationsCreated).ToArray();
                scope.Complete();
            }
